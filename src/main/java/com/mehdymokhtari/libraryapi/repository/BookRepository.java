...
public interface BookRepository extends JpaRepository<Book, Long>, JpaSpecificationExecutor<Book> {
    ...
    @Query("SELECT COUNT(b) > 0 FROM Book b WHERE b.id = :id AND b.status = :status AND b.isDeleted = false")
    boolean isBookWithStatus(@Param("id") Long id, @Param("status") BookStatus status);

    @Query("SELECT COUNT(b) > 0 FROM Book b WHERE b.isbn = :isbn AND b.id != :id AND b.isDeleted = false")
