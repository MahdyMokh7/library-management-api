...
public interface BookRepository extends JpaRepository<Book, Long>, JpaSpecificationExecutor<Book> {
  ...
  boolean existsByIdAndStatusAndDeletedFalse(Long id, BookStatus status);

  Optional<Book> findByIdAndDeletedFalse(Long id);

  @Query(
      "SELECT COUNT(b) > 0 FROM Book b WHERE b.id = :id AND b.status = :status AND b.deleted = false")
  boolean isBookWithStatus(@Param("id") Long id, @Param("status") BookStatus status);

  @Query(
      "SELECT COUNT(b) > 0 FROM Book b WHERE b.isbn = :isbn AND b.id != :id AND b.deleted = false")
