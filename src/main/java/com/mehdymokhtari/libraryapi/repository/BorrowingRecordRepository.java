...
public interface BorrowingRecordRepository extends JpaRepository<BorrowingRecord, Long> {
  ...
  Optional<BorrowingRecord> findByBookIdAndStatus(
      @Param("bookId") Long bookId, @Param("status") BorrowingStatus status);

  List<BorrowingRecord> findByBookIdAndStatusOrderByBorrowedDateDesc(
      Long bookId, BorrowingStatus status);
  ...
  @Query(
      "SELECT COUNT(br) > 0 FROM BorrowingRecord br WHERE br.book.id = :bookId AND br.status = 'BORROWED'")
  boolean isBookCurrentlyBorrowed(@Param("bookId") Long bookId);

  @Query(
      "SELECT br FROM BorrowingRecord br WHERE br.book.id = :bookId ORDER BY br.borrowedDate DESC")
