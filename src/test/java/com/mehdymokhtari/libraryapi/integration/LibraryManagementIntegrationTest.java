...
class LibraryManagementIntegrationTest {
    ...
    void shouldCreateAndRetrieveBookEndToEnd() throws Exception {
        ...
        ApiResponse<BookResponse> apiResponse = objectMapper.readValue(
                createResponse,
                objectMapper.getTypeFactory().constructParametricType(ApiResponse.class, BookResponse.class));
    ...
    void shouldGetBorrowingHistoryEndToEnd() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/borrowings/book/{bookId}", bookId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void shouldGetBorrowingHistoryByBorrowerNameEndToEnd() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/borrowings/borrower/{name}", "John"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data").isArray())
                .andExpect(jsonPath("$.data.length()").value(2))
    ...
    void shouldThrowExceptionWhenBorrowingAlreadyBorrowedBookEndToEnd() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/borrowings/borrow")
                ...
                .andExpect(jsonPath("$.message").value("Item with ID " + bookId + " is not available for borrowing"));
    }
    ...
    void shouldThrowExceptionWhenReturningNotBorrowedBookEndToEnd() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/borrowings/return")
                ...
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error").value("Conflict"))
                .andExpect(jsonPath("$.message").value("Item with ID " + bookId + " is not currently borrowed"));
    }
