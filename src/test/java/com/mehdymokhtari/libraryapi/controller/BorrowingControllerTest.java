...
class BorrowingControllerTest {
  ...
  void shouldReturn404WhenBorrowingNonExistingBook() throws Exception {
    ...
    when(borrowingService.borrowBook(any(BorrowRequest.class)))
        .thenThrow(
            new com.mehdymokhtari.libraryapi.exception.ResourceNotFoundException("Book", 999L));
    ...
    when(borrowingService.borrowBook(any(BorrowRequest.class)))
        .thenThrow(
            new com.mehdymokhtari.libraryapi.exception.BookNotAvailableException(
                "Book with ID 1 is not available for borrowing"));
    ...
    when(borrowingService.returnBook(any(ReturnRequest.class)))
        .thenThrow(
            new com.mehdymokhtari.libraryapi.exception.BusinessException(
                "Book with ID 1 is not currently borrowed"));
